...
public final class BountyHunterManager {
	...
	private static boolean allowDeath(PlayerEntity victim, PlayerEntity killer, Identifier reason) {
		...
		if (bountyTarget) {
			shop.addToBalance(GexpressConfig.getBountyHunterRewardGold());
			PlayerShopComponent.KEY.sync(hunter);
			clearHunter(hunter.getUuid());
			sendClear(hunter);
		} else {
			int penalty = Math.min(50, Math.max(0, shop.balance));
			if (penalty > 0) {
				shop.addToBalance(-penalty);
				PlayerShopComponent.KEY.sync(hunter);
			}
		}
		return true;
	...
	private static boolean isBountyCandidateSide(GameWorldComponent game, ServerPlayerEntity player) {
		if (game == null || player == null) return false;
		Role role = game.getRole(player);
		return role != null && role.isInnocent() && !game.canUseKillerFeatures(player) && !role.canUseKiller();
	}
