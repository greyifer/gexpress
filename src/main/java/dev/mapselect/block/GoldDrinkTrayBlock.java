...
public class GoldDrinkTrayBlock extends DrinkTrayBlock {
	public static final MapCodec<GoldDrinkTrayBlock> CODEC = createCodec(GoldDrinkTrayBlock::new);
	public static final BooleanProperty PAID = BooleanProperty.of("paid");

	public GoldDrinkTrayBlock(Settings settings) {
		super(settings);
	...
	private ActionResult takeItem(BlockState state, World world, PlayerEntity player, GoldBeveragePlateBlockEntity plate) {
		List<ItemStack> tray = plate.getStoredItems();
		if (tray.isEmpty()) return ActionResult.SUCCESS;
		int matchingItems = countMatchingTrayItems(player, tray);
		boolean thirsty = ModifierUtils.has(player, MapSelectModifiers.THIRSTY_ID);
		if (!thirsty && matchingItems > 0) return ActionResult.PASS;
		if (thirsty && matchingItems >= GexpressConfig.getThirstyDrinkLimit()) return ActionResult.SUCCESS;
		int price = GexpressConfig.getGoldDrinkTrayPrice();
		if (state.get(PAID) && !player.isCreative() && price > 0 && !tryCharge(player, price)) {
			return ActionResult.SUCCESS;
		}
		ItemStack randomItem = tray.get(world.random.nextInt(tray.size())).copy();
	...
	private static int countMatchingTrayItems(PlayerEntity player, List<ItemStack> tray) {
		int count = 0;
		for (int slot = 0; slot < player.getInventory().size(); slot++) {
			ItemStack stack = player.getInventory().getStack(slot);
			if (stack.isEmpty()) continue;
			for (ItemStack trayItem : tray) {
				if (!trayItem.isEmpty() && stack.isOf(trayItem.getItem())) {
