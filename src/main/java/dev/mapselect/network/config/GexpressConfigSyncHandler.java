	...
	public static void register() {
		...
		ServerPlayNetworking.registerGlobalReceiver(GexpressConfigSyncPayload.ID,
			(payload, context) -> {
				...
				if (!GexpressPermissions.canEditGameTab(sender)) {
					MapSelect.LOGGER.warn("Ignoring gexpress config sync from {} (missing config_edit permission)",
				...
				server.execute(() -> {
					...
					GexpressConfig.apply(payload.c4Price(), payload.c4FuseSeconds(),
						...
						payload.painterPaintDurationSeconds(), payload.painterBodyCooldownSeconds(),
						payload.painterPlayerCooldownSeconds(), payload.painterDoorwayDurationSeconds(),
						payload.painterDoorwayCooldownSeconds(),
						payload.painterMaxPaintedBodies(), payload.painterMaxPaintedPlayers(),
						...
						payload.lastStandEnabled(), payload.itemPickupPolicy(),
						...
						payload.seerCompareCooldownSeconds(), payload.seerCompareDeathsRequired(),
						payload.seerCompareRange(),
						payload.twinsSwapCooldownSeconds(), payload.twinsBodyDifferentRole(),
					...
					GexpressConfig.save();
					MapSelect.LOGGER.info("G'Express config updated by {}: price={}, fuse={}s, firstBeep={}s, wrongWire={}%, grenade={}, maxKillers={}, maxVigilantes={}, maxNeutrals={}, maxModifiers={}, medicShieldCooldown={}s, medicShieldKnifeBreaks={}, silent=[duration {}s, cooldown {}s, alpha {}], warlock=[markCooldown {}s, killCooldown {}s], juggernaut=[initial {}s, reduction {}s, min {}s, shieldRecharge {}s], trickster=[duration {}s, cartUses {}], puppetmaster=[duration {}s, cooldown {}s, random {}], pelican=[eatCooldown {}s], snitch=[tasks {}, warnRemaining {}], timeMaster=[rewind {}s, cooldown {}s, uses {}, freezeDuration {}s, freezeCooldown {}s, freezeUses {}, freezeRange {}], c4Back=[{}, {}, {}; rot {}, {}, {}; slant {}; scale {}]",
						sender.getName().getString(),
		...
		ServerPlayNetworking.registerGlobalReceiver(GexpressTaskConfigPayload.ID,
			(payload, context) -> {
				ServerPlayerEntity sender = context.player();
				if (!GexpressPermissions.canEditGameTab(sender)) {
					MapSelect.LOGGER.warn("Ignoring gexpress task config sync from {} (missing config_edit permission)",
						sender.getName().getString());
					return;
				}
		...
		ServerPlayNetworking.registerGlobalReceiver(GexpressDevTuningPayload.ID,
			(payload, context) -> {
				ServerPlayerEntity sender = context.player();
				if (!GexpressPermissions.canEditDevTab(sender)) {
					MapSelect.LOGGER.warn("Ignoring gexpress dev tuning sync from {} (missing dev_menus permission)",
						sender.getName().getString());
					return;
				}
		...
		ServerPlayNetworking.registerGlobalReceiver(PuppetmasterConfigPayload.ID,
			(payload, context) -> {
				ServerPlayerEntity sender = context.player();
				if (!GexpressPermissions.canEditGameTab(sender)) {
					MapSelect.LOGGER.warn("Ignoring puppetmaster config sync from {} (missing config_edit permission)",
						sender.getName().getString());
					return;
				}
