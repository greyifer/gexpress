package dev.mapselect.network.preset;
...
public final class GexpressPresetsSyncHandler {
	...
	public static void register() {
		...
		ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
			...
			server.execute(() -> {
				if (canViewMaps(player)) sendPresetsTo(player);
				if (canViewTrainCarts(player)) sendTrainPresetsTo(player);
			});
		});
	...
	private static boolean canViewMaps(ServerPlayerEntity player) {
		return GexpressPermissions.canViewMapsTab(player);
	}

	private static boolean canEditMaps(ServerPlayerEntity player) {
		return GexpressPermissions.canEditMapsTab(player);
	}

	private static boolean canViewTrainCarts(ServerPlayerEntity player) {
		return GexpressPermissions.canViewTrainCartsTab(player);
	}

	private static boolean canEditTrainCarts(ServerPlayerEntity player) {
		return GexpressPermissions.canEditTrainCartsTab(player);
	}
