...
public final class ClientAbilityCooldownHud {
	...
	private static final Identifier ICON_TWINS = ICON_MASQUERADE;
	...
	private static final Identifier ICON_PICKPOCKET = null;
	...
	private static final Map<String, Float> VISUAL_PROGRESS = new HashMap<>();
	...
	private static void render(DrawContext context, RenderTickCounter tickCounter) {
		...
		boolean hasIcons = bars.stream().anyMatch(bar -> bar.icon() != null);
		int iconSlotWidth = hasIcons ? iconSize + iconBarGap : 0;
		int barWidth = scaled(BASE_BAR_WIDTH, scale);
		int barHeight = Math.max(4, scaled(BASE_BAR_HEIGHT, scale));
		int barSpacing = Math.max(scaled(BASE_BAR_SPACING, scale), hasIcons ? iconSize + 2 : barHeight + 2);
		...
		int x = hotbarRight + hotbarSideGap + iconSlotWidth + GexpressConfig.getAbilityHudOffsetX();
		x = clamp(x, 2 + iconSlotWidth, context.getScaledWindowWidth() - barWidth - TIMER_WIDTH - 4);

		int hotbarTop = context.getScaledWindowHeight() - HOTBAR_HEIGHT;
		int rowHeight = hasIcons ? iconSize : barHeight;
		int totalHeight = rowHeight + (bars.size() - 1) * barSpacing;
		int startY = hotbarTop - hotbarTopGap - totalHeight + GexpressConfig.getAbilityHudOffsetY();
		startY = clamp(startY, 2, context.getScaledWindowHeight() - totalHeight - 2);
		int y = startY + (rowHeight - barHeight) / 2;
		TextRenderer text = client.textRenderer;
		List<String> activeVisualKeys = new ArrayList<>();
		for (int i = 0; i < bars.size(); i++) {
			String visualKey = visualKey(client, bars.get(i), i);
			activeVisualKeys.add(visualKey);
			drawBar(context, text, bars.get(i), visualKey, x, y + i * barSpacing,
				hasIcons ? iconSize : 0, hasIcons ? iconBarGap : 0, barWidth, barHeight);
		}
		VISUAL_PROGRESS.keySet().removeIf(key -> !activeVisualKeys.contains(key));
	}
	...
	private static List<AbilityBar> barsFor(MinecraftClient client) {
		...
		if (MapSelectRoles.MEDIC_ID.equals(roleId)) {
		...
		} else if (MapSelectRoles.PAINTER_ID.equals(roleId)) {
			bars.add(syncedOrReady(AbilityCooldownPayload.PAINTER_BODY, ICON_BORROWED_SKIN,
				Math.max(GexpressConfig.getPainterBodyCooldownSeconds(),
					GexpressConfig.getPainterPlayerCooldownSeconds()) * 20L, 0xFFC65BFF, 0xFF512069));
			bars.add(syncedOrReady(AbilityCooldownPayload.PAINTER_DOORWAY, ICON_MASQUERADE,
				GexpressConfig.getPainterDoorwayCooldownSeconds() * 20L, 0xFFB45BFF, 0xFF3D1761, "", true));
			...
			bars.add(syncedCharge(AbilityCooldownPayload.SEER_COMPARE, ICON_SEER,
				GexpressConfig.getSeerCompareDeathsRequired(), 0xFFD94B66, 0xFF6E1D31));
		} else if (MapSelectRoles.TWINS_ID.equals(roleId)) {
			bars.add(syncedOrReady(AbilityCooldownPayload.TWINS_SWAP, ICON_TWINS,
				GexpressConfig.getTwinsSwapCooldownSeconds() * 20L, 0xFF72D6FF, 0xFF23506E));
	...
	private static Identifier copycatStoredIcon(Identifier roleId) {
		...
		if (MapSelectRoles.TWINS_ID.equals(roleId)) return ICON_TWINS;
	...
	private static ExternalAbility externalAbility(Identifier roleId) {
		...
		if ("kinswathe".equals(namespace) && any(path,
			...
			return new ExternalAbility(Identifier.of("kinswathe", "ability"), null, 60L * 20L);
		}
		if ("noellesroles".equals(namespace) && any(path,
				"recaller", "phantom", "vulture", "infected", "swapper", "voodoo", "guesser")) {
			return new ExternalAbility(Identifier.of("noellesroles", "ability"), null, 60L * 20L);
		}
		if ("starexpress".equals(namespace) && "starstruck".equals(path)) {
			return new ExternalAbility(Identifier.of("starexpress", "ability"), null, 90L * 20L);
		}
		if ("stupid_express".equals(namespace) && any(path, "thief", "necromancer")) {
			return new ExternalAbility(Identifier.of("stupid_express", "cooldown"), null, 90L * 20L);
		}
		return null;
	}
	...
	private static AbilityBar syncedOrReady(String key, Identifier icon, long readyTotalTicks, int color, int darkColor) {
		return syncedOrReady(key, icon, readyTotalTicks, color, darkColor, "", false);
	}

	private static AbilityBar syncedOrReady(String key, Identifier icon, long readyTotalTicks, int color,
			int darkColor, String extraText, boolean secondaryKey) {
		SyncedCooldown synced = SYNCED.get(key);
		if (synced == null) return cooldown(icon, 0L, readyTotalTicks, color, darkColor, extraText, secondaryKey);
		MinecraftClient client = MinecraftClient.getInstance();
		if (isCreativeRolePreview(client) && !synced.draining()) {
			SYNCED.remove(key);
			return cooldown(icon, 0L, readyTotalTicks, color, darkColor, extraText, secondaryKey);
		}
		...
		return synced.draining()
			? draining(icon, remaining, synced.totalTicks(), color, darkColor, secondaryKey)
			: cooldown(icon, remaining, synced.totalTicks(), color, darkColor, extraText, secondaryKey);
	}
	...
	private static AbilityBar syncedCharge(String key, Identifier icon, int configuredRequired, int color, int darkColor) {
		SyncedCooldown synced = SYNCED.get(key);
		int required = Math.max(1, synced != null && synced.maxUses() > 0 ? synced.maxUses() : configuredRequired);
		int charges = Math.max(0, Math.min(required, synced == null ? 0 : synced.usesRemaining()));
		MinecraftClient client = MinecraftClient.getInstance();
		if (isCreativeRolePreview(client)) charges = required;
		return charge(icon, charges, required, color, darkColor);
	}
	...
	private static AbilityBar charge(Identifier icon, int charges, int required, int color, int darkColor) {
		int max = Math.max(1, required);
		int filled = Math.max(0, Math.min(max, charges));
		return new AbilityBar(icon, 0L, filled / (float) max, color, darkColor,
			filled + "/" + max, false, 0, 0, 0, 0);
	}

	private static void drawBar(DrawContext context, TextRenderer text, AbilityBar bar, String visualKey, int x, int y,
			int iconSize, int iconBarGap, int barWidth, int barHeight) {
		int iconX = x - iconBarGap - iconSize;
		int iconY = y - (iconSize - barHeight) / 2;
		if (bar.icon() != null && iconSize > 0) {
		...
		if (bar.segments() > 0) {
			...
			int fill = Math.round(innerWidth * smoothedProgress(visualKey, bar.progress()));
			if (fill > 0) {
				int fillX = x + 1;
	...
	private static String visualKey(MinecraftClient client, AbilityBar bar, int index) {
		Role localRole = localRole(client);
		Identifier role = localRole == null ? null : localRole.identifier();
		return (role == null ? "unknown" : role.toString()) + ":" + index + ":"
			+ (bar.icon() == null ? "none" : bar.icon().toString()) + ":"
			+ bar.color() + ":" + bar.darkColor() + ":" + bar.secondaryKey();
	}

	private static float smoothedProgress(String key, float target) {
		float clampedTarget = Math.max(0.0F, Math.min(1.0F, target));
		if (key == null || key.isBlank()) return clampedTarget;
		Float previous = VISUAL_PROGRESS.get(key);
		if (previous == null) {
			VISUAL_PROGRESS.put(key, clampedTarget);
			return clampedTarget;
		}
		float next = previous + (clampedTarget - previous) * 0.18F;
		if (Math.abs(next - clampedTarget) < 0.004F) next = clampedTarget;
		VISUAL_PROGRESS.put(key, next);
