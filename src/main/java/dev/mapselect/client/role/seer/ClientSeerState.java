package dev.mapselect.client.role.seer;
...
public final class ClientSeerState {
	...
	public static void register() {
		...
		ClientPlayNetworking.registerGlobalReceiver(SeerSelectionPayload.ID, (payload, context) ->
			context.client().execute(() -> ClientAbilityTargetState.setSeerSelection(
				payload.active() ? payload.targetId() : null)));
	...
	private static void tick(MinecraftClient client) {
		...
		if (down && !wasCompareDown && ClientPlayNetworking.canSend(SeerCompareUsePayload.ID)) {
			ClientPlayNetworking.send(new SeerCompareUsePayload(ClientAbilityTargetState.currentTargetId()));
		}
		wasCompareDown = down;
	}
