package dev.mapselect.client.role.copycat;
...
public final class ClientCopycatState {
	...
	private static boolean wasTertiaryDown;
	...
	private static void tick(MinecraftClient client) {
		...
		if (isBorrowingAbility()) {
			wasPrimaryDown = false;
			wasSecondaryDown = false;
			KeyBinding tertiaryBinding = ClientAbilityKeys.tertiaryBinding();
			boolean tertiary = tertiaryBinding != null && ClientAbilityKeys.isDown(client, tertiaryBinding);
			if (tertiary && !wasTertiaryDown && ClientPlayNetworking.canSend(CopycatActionPayload.ID)) {
				ClientPlayNetworking.send(CopycatActionPayload.cancelActive());
			}
			wasTertiaryDown = tertiary;
			return;
		}
		wasTertiaryDown = false;
	...
	public static void cancelActive() {
		if (isBorrowingAbility() && ClientPlayNetworking.canSend(CopycatActionPayload.ID)) {
			ClientPlayNetworking.send(CopycatActionPayload.cancelActive());
		}
	}

	public static boolean shouldSuppressBorrowedSecondary(MinecraftClient client, KeyBinding binding, boolean down) {
