package dev.mapselect.client.role.mafia;
...
import dev.mapselect.client.screen.GodfatherRecruitScreen;
...
public final class ClientMafiaState {
	...
	public static boolean shouldGlow(UUID playerId) {
		...
		return shouldUseFamilyInstinct() && familyIds.contains(playerId);
	}
	...
	public static boolean shouldUseFamilyInstinct() {
		MinecraftClient client = MinecraftClient.getInstance();
		return client != null && client.player != null && WatheClient.instinctKeybind != null
			&& ClientAbilityKeys.isDown(client, WatheClient.instinctKeybind)
			&& isLocalMafia(client) && isRoundRunning(client)
	...
	private static boolean shouldUseBlackWhite(MinecraftClient client) {
		return isLocalMafia(client) && isRoundRunning(client) && client != null && client.player != null
			&& !ClientCupidState.isCupidOverlayVisible()
			&& !ClientVultureState.isLocalStashed(client)
			&& GameFunctions.isPlayerAliveAndSurvival(client.player);
	}
	...
	private static void updateMafiaIntro(MinecraftClient client) {
		...
		if (ClientCupidState.isCupidOverlayVisible()) return;
